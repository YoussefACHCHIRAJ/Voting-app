<div x-cloak x-data="{ isOpen: false }" x-init="Livewire.on('exerciceWasUpdated', () => isOpen = false)" @showing-edit-exercice-modal.window="isOpen=true" x-show="isOpen"
        ...
        <div class="min-h-full ">
            <div x-transition x-show.transition.origin.bottom.duration.350ms="isOpen"
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    ...
                    <h3 class="text-center text-lg font-medium text-gray-900">Edit Exercice</h3>
                    <p class="text-xs text-center mt-4 text-gray-500">You have one hour to edit your exercice from the time
                        you created it</p>
                    <form wire:submit.prevent="editExercice" action="#" method="POST" class="space-y-4 px-4 py-6">
                        <div>
                        ...
                        <div>
                            <textarea wire:model="description" name="exercice" id="exercice" cols="30" rows="4"
                                class="w-full bg-gray-100 rounded-xl placeholder:text-gray-900 text-sm px-4 py-2 border-none"
                                placeholder="Describe your exercice">{{ $description }}</textarea>
                            @error('description')
                                <p class="text-red text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
